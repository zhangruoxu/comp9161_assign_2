...
runTests exe testdir = do
    ...
    marks <- foreach tests $ (\test -> do
      ...
      (exit, out, err) <- readProcessWithExitCode exe (flags ++ [test]) ""
  ...
  where
    ...
    doCheck ext name test out = do
      ...
      if v
        then do
          diff <- getDiff (lines out) <$> lines <$> readFile (test `replaceExtension` ".out")
...
getFlags filename = do
  ...
  if v then do
         str <- concatMap words <$> lines <$> readFile filename
...
main = do
  ...
  when (v == [ "--help" ]) $ do
     putStrLn $ "Liam's Haskelly Test Runner v0.1. \n" ++
                ...
                "  Usage: ./run_tests.sh [--no-colour] [program_to_test] [test_folder_location]\n\n" ++
